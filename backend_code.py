...
# FastAPI app setup
# ----------------------------------------------------------------------------
app = FastAPI(title="Diabetic Risk Prediction - Extended Features", version="1.1.0")
...
# ----------------------------------------------------------------------------
# Data models (Pydantic v2 style) -- extended to match "Selected Feature Ranges and Units"
# ----------------------------------------------------------------------------
class VitalSigns(BaseModel):
    # Adjusted systolic range to match provided file (77 - 219 mmHg)
    systolic_bp: Optional[int] = Field(None, ge=77, le=219, description="Systolic blood pressure (mmHg)")
    # No explicit diastolic in the file; keep sensible defaults
...
class PatientData(BaseModel):
    ...
    Fields and ranges were expanded/adjusted to reflect the provided "Selected Feature Ranges and Units" document.
    All new optional lab/test fields are included so the external model can benefit from them.
    """

    patient_id: str = Field(..., description="Stable ID used to group history")
    # Age range adjusted to 13-91 years (from the uploaded doc)
    age: int = Field(..., ge=13, le=91)
    sex: Literal["male", "female", "other"]

    # BMI adjusted to documented min/max
    bmi: Optional[float] = Field(None, ge=15.45, le=64.89, description="kg/m² - Body Mass Index")

    # HbA1c remains optional but range is kept practical
    hba1c: Optional[float] = Field(None, ge=0, le=20, description="Hemoglobin A1c (%)")

    vitals: Optional[VitalSigns] = None

    # Additional features from the uploaded doc (optional; units described in doc)
    diabetes_age: Optional[int] = Field(None, ge=-1, le=60, description="Years since diabetes diagnosis (-1 unknown)")
    height_m: Optional[float] = Field(None, ge=1.34, le=1.86, description="Height in metres")

    # Laboratory / test values (optional) with ranges from the doc
    test_alt_u_l: Optional[float] = Field(None, ge=5.7, le=811, description="Alanine aminotransferase (U/L)")
    test_albumin_g_l: Optional[float] = Field(None, ge=26.2, le=49.7, description="Serum albumin (g/L)")
    test_calcium_mmol_l: Optional[float] = Field(None, ge=1.58, le=2.69, description="Serum calcium (mmol/L)")
    test_creatinine_umol_l: Optional[float] = Field(None, ge=8, le=596, description="Serum creatinine (µmol/L)")
    test_cystatin_c_mg_l: Optional[float] = Field(None, ge=0.388, le=6.512, description="Cystatin C (mg/L)")
    test_hct: Optional[float] = Field(None, ge=0.2252, le=0.5331, description="Hematocrit (L/L)")
    test_hb_g_l: Optional[float] = Field(None, ge=67.2, le=178.1, description="Hemoglobin (g/L)")
    test_uacr_mg_g: Optional[float] = Field(None, ge=1, le=5514, description="Urine albumin-to-creatinine ratio (mg/g)")
    test_mxd_x10e9_l: Optional[float] = Field(None, ge=0.3, le=3.111, description="Mixed white cell count (×10⁹/L)")
    test_phosphorous_mmol_l: Optional[float] = Field(None, ge=0.48, le=2.19, description="Serum phosphorus (mmol/L)")
    test_rbc_x10e12_l: Optional[float] = Field(None, ge=2.84, le=8.083, description="Red blood cell count (×10¹²/L)")
    test_urea_mmol_l: Optional[float] = Field(None, ge=1.1, le=34.3, description="Blood urea (mmol/L)")

    weight_kg: Optional[float] = Field(None, ge=34.3, le=160.4, description="Body weight (kg)")
...
# ----------------------------------------------------------------------------
# Risk scores now target the eight specified diabetes-related complications
# ----------------------------------------------------------------------------
class RiskScores(BaseModel):
    """Per-complication risk scores in [0, 1]."""

    hypertension: Optional[float] = Field(None, ge=0.0, le=1.0)
    obesity: Optional[float] = Field(None, ge=0.0, le=1.0)
    dyslipidemia: Optional[float] = Field(None, ge=0.0, le=1.0)
    metabolic_syndrome: Optional[float] = Field(None, ge=0.0, le=1.0)
    diabetic_foot: Optional[float] = Field(None, ge=0.0, le=1.0)
...
async def call_model_api(patient: PatientData) -> Dict[str, Any]:
    ...
    It includes the expanded set of features present in the uploaded ranges/units doc.
    """

    # Build a flat feature dict. Use explicit keys that downstream model expects.
    features: Dict[str, Any] = {
        ...
        "hba1c": patient.hba1c,
        # vitals
        "systolic_bp": getattr(patient.vitals, "systolic_bp", None) if patient.vitals is not None else None,
        "diastolic_bp": getattr(patient.vitals, "diastolic_bp", None) if patient.vitals is not None else None,
        "heart_rate": getattr(patient.vitals, "heart_rate", None) if patient.vitals is not None else None,
        # documented additional features
        "diabetes_age": patient.diabetes_age,
        "height_m": patient.height_m,
        "weight_kg": patient.weight_kg,
        "test_alt_u_l": patient.test_alt_u_l,
        "test_albumin_g_l": patient.test_albumin_g_l,
        "test_calcium_mmol_l": patient.test_calcium_mmol_l,
        "test_creatinine_umol_l": patient.test_creatinine_umol_l,
        "test_cystatin_c_mg_l": patient.test_cystatin_c_mg_l,
        "test_hct": patient.test_hct,
        "test_hb_g_l": patient.test_hb_g_l,
        "test_uacr_mg_g": patient.test_uacr_mg_g,
        "test_mxd_x10e9_l": patient.test_mxd_x10e9_l,
        "test_phosphorous_mmol_l": patient.test_phosphorous_mmol_l,
        "test_rbc_x10e12_l": patient.test_rbc_x10e12_l,
        "test_urea_mmol_l": patient.test_urea_mmol_l,
    }
    ...
    # Expect the model to provide risk_scores for the eight complications and a shap mapping
